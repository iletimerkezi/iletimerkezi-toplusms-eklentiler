...
class ControllerSaleSms extends Controller {
	...
	public function index() {
		...
		if(!empty($this->data['iletimerkezisms']['iletimerkezi_username'])) {
        	$iletimerkezi_username = $this->data['iletimerkezisms']['iletimerkezi_username'];
        } else {
        	$iletimerkezi_username = '';
		}

        if(!empty($this->data['iletimerkezisms']['iletimerkezi_password'])) {
            $iletimerkezi_password = $this->data['iletimerkezisms']['iletimerkezi_password'];
        } else {
            $iletimerkezi_password = '';
        }
		

        if(empty($iletimerkezi_username)||empty($iletimerkezi_password)) {
        	$this->data['sender'] = 'Sms göndermek ve bakiyenizin gözükebilmesi için üyelik bilgilerinizi giriniz.';
        } else {        	
        	$this->load->library('sms');
			$sms = new Sms();
			$res = $sms->getSender($iletimerkezi_username,$iletimerkezi_password);
				...
				switch ($this->request->post['to']) {
					case 'newsletter':
						...
						foreach ($results as $result) {
							...
							$numbers = $this->clearNumber($numbers);
