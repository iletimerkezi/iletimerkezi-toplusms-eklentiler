...
class ControllerModulePhoneVerification extends Controller {
    ...
    public function getrepl($ret=0) {
        ...
        if ($this->ocversion==1) {
        	...
        	$repl = '<button id="journal-checkout-confirm-button" data-loading-text=" '. $this->journal2->settings->get('one_page_lang_loading_text', 'Loading..').'" class="button confirm-button">'. $this->journal2->settings->get('one_page_lang_confirm_order', 'Confirm Order').'</button>';
        }
